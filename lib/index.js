...
connect = function(mimosaConfig, options, next) {
  ...
  io = options.socketio != null ? (logger.debug("Using user's socketio"), options.socketio) : (logger.debug("Using module's socketio"), socketio = require('socket.io'), io = socketio.listen(options.userServer), io.enable('browser client minification'), io.enable('browser client etag'), io.set('log level', 1), io);
